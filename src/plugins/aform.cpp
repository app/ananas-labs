...
aForm::init()
{
	...
	if ( !mdObj.isNull() && md ) {
		...
		if ( !ui.isEmpty() ) {
			...
			b.close();
			b.open(IO_ReadOnly );
			aLog::print(aLog::MT_INFO, tr("aForm creating form from ui"));
			form = QWidgetFactory::create( &b );
			if (form) {
				aLog::print(aLog::MT_INFO, tr("aForm form create from ui ok"));
			} else {
				aLog::print(aLog::MT_ERROR, tr("aForm::init  Can't create screen form from ui. buffer='%1'").arg(b.buffer()));
			}
			b.close();
		}
	}
	if (form) {
		...
		{
			aLog::print(aLog::MT_DEBUG, tr("aForm::init window found in windowlist, removing..."));
			engine->wl->remove( objid, db_uid );
		}
		else
		{
			aLog::print(aLog::MT_DEBUG, tr("aForm::init window not found in windowlist"));
		}
		engine->wl->insert( objid, mw, db_uid );
		aLog::print(aLog::MT_DEBUG, tr("aForm::init insert window in windowlist"));
		...
		{
			aLog::print(aLog::MT_ERROR, tr("aForm do not inherits aWidget!"));
		...
		//	engine->project.addObject(this);
			...
			aLog::print(aLog::MT_DEBUG, tr("aForm load form module script"));
			QStringList lst = engine->project.interpreter()->functions(this);
			for(uint i=0; i<lst.count();i++)
			{
				aLog::print(aLog::MT_DEBUG, tr("aForm defined function %1").arg(lst[i]));
			}
		}
		else
		{
			aLog::print(aLog::MT_DEBUG, tr("aForm form module script is empty"));
		}
 ...
 *	Параметр modal определяет является ли форма модальной.
 *	\see close
 *\~
*/
void
aForm::Show(bool modal)
{
	if ( form )
	{
		if ( FormHasFunction("on_formstart") )
		{
		...
		if(!modal)
		{
			...
			connect( form, SIGNAL(closed()), this, SLOT(Close()) );
		}
		else
		{
			dlg = new EventDialog(parentWidget, "form dialog", true);
			form->reparent(dlg, QPoint(0,0), true);
			dlg->show();
			connect( dlg, SIGNAL(closed()), this, SLOT(Close()) );
		}
    }
}
...
void
aForm::show() {
	aLog::print(aLog::MT_DEBUG,tr("Deprecated method call: aForm::show()"));
	this->Show();
}
...
aForm::Close() {
	...
	if( form ) {
		if( form->isShown() ) {
			...
			form->hide();
#endif
			if ( mainWidget->inherits("wCatalogue") )
			{
				form->hide();
			}
...
void
aForm::close() {
	aLog::print(aLog::MT_DEBUG,tr("Deprecated method call: aForm::close(). Use aForm::Close() instead"));
	this->Close();
}
...
int
aForm::turn_on(){
	aLog::print(aLog::MT_DEBUG,tr("Deprecated method call: aForm::turn_on()"));
	return SignIn();
}
...
aForm::SignIn(){
	...
	if(res.type()==QSArgument::Variant)
	{
		...
		{
			aLog::print(aLog::MT_INFO, tr("aForm conduct: function on_conduct() return false, document not conducted"));
			return 0;
		}
	}
...
int
aForm::turn_off(){
	aLog::print(aLog::MT_DEBUG,tr("Deprecated method call: aForm::turn_off()"));
	return SignOut();
}
...
aForm::DBValue(const QString &name)
{
	...
	{
		aLog::print(aLog::MT_ERROR, tr("aForm not found widget with name %1").arg(name));
	}
	return res;
}
...
aForm::SetColumnReadOnly(const QString &tname, int numCol, bool ro)
{
	...
	{
		aLog::print(aLog::MT_ERROR, tr("aForm not found widget with name %1").arg(tname));
	}
...
aForm::TabNewLine(const QString &tname)
{
       ...
       {
               aLog::print(aLog::MT_ERROR, tr("aForm not found wDBTabe widget with name %1").arg(tname));
       }
}
...
{
	return tabValue(tname,row,col,false).toString();
}
...
aForm::tabValue(const QString &tname, int row, int col, bool dbval)
{
	...
	if ( w  && !strcmp(w->className(),"wDBTable"))
	{
		wDBTable *wdb = (wDBTable*)w;
			...
			{
				aLog::print(aLog::MT_ERROR, tr("aForm table value: column number '%1' out of range").arg(col));
			}
	}
	else
	{
...
aForm::SetTabValue(const QString &tname, const QString &colname, int row, QVariant value)
{
	...
	{
		aLog::print(aLog::MT_ERROR, tr("aForm not found wDBTable widget with name %1").arg(tname));
	}
...
void
aForm::done( int rc ){
	aLog::print(aLog::MT_DEBUG,tr("Deprecated method call: aForm::done(...)"));
	rc=rc;
...
aForm::setfocus(QString fname){
	aLog::print(aLog::MT_DEBUG,tr("Deprecated method call: aForm::setfocus()"));
//	QWidget *w;
 ...
 *
 *	Функция вызывается при попытке закрытия окна,
 *	вызывает функцию Ananas API on_formstop, текущей формы,
 *	в случае успеха возвращает значение полученное из скрипта,
 *	иначе - истину.
 *\_ru
 */
bool
aForm::on_form_close(){
	if ( !FormHasFunction(QString("on_formstop")) ) {
		aLog::print(aLog::MT_DEBUG, QObject::tr("on_formstop() definition wasn't found. Form should be closed."));
		return true;
	}
...
/*
 *\ru
 *	\brief	Проверяет присутствие в коде модуля формы определения функции с заданным именем.
 *	\return	true если функция определена или false в противном случае.
 *\_ru
 */
bool
...
aForm::on_valueChanged( const QString & name, const QVariant & val )
{
	...
	if ( FormHasFunction("on_valuechanged") )
	{
...
aForm::on_tabvalueChanged(int row, int col)
{
	...
	if ( FormHasFunction("on_tabupdate") )
	{
		QValueList<QVariant> lst;
		lst << row;
		lst << col;
...
aForm::on_dbtablerow( QSqlRecord *r )
{
	...
	if ( FormHasFunction("on_tablerow") ) {
...
aForm::on_tabselected( Q_ULLONG uid )
{
	emit selected( uid );
	if ( closeAfterSelect ) {
		aLog::print(aLog::MT_DEBUG, QObject::tr("aForm::on_tabselected(...),   closeAfterSelect is true"));
		Close();
	} else {
		aLog::print(aLog::MT_DEBUG, QObject::tr("aForm::on_tabselected(...),   closeAfterSelect is false"));
	}
}
...
aForm::on_tablerow( Q_ULLONG uid )
{
	...
	if ( FormHasFunction("on_tabrowselected") ) {
...
aForm::SetReadOnly ( bool status )
{
	...
	if ( mainWidget )
	{
		if(status)
		{
			aLog::print(aLog::MT_INFO, tr("aForm set read only"));
		}
		else
		{
			aLog::print(aLog::MT_INFO, tr("aForm set read/write"));
		}
...
aForm::SelectByCurrent(aObject *doc)
{
	if(mainWidget)
	{
		...
		aLog::print(aLog::MT_DEBUG, QObject::tr("aForm SelectByCurrent %1 ended with %1").arg(mainWidget->uid()).arg(res));
	}
}
 ...
 *	Maximize window
 *\~russian
 * 	\brief ScriptAPI. Распахивает окно на все рабочее прастранство
 *\~
 */
void aForm::maximize(bool m)
{
#if (QT_VERSION-0 >= 0x030300) // setWindowState() появился в qt3.3
	if(form)
	{
		if(m)
			form->setWindowState(WindowMaximized);
		else
			form->setWindowState(~WindowMaximized);
	}
#endif
