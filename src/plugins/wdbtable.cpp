...
//	verticalHeader()->hide();
	...
	aLog::print(aLog::MT_DEBUG, tr("wDBTable init ok"));
...
//	proverka na nalichie field in metadata
	for(i=0; i<il.count(); i++)
	{
		...
		if(i<fl.count())
		{
			if(str!=fl[i])
			{
				aLog::print(aLog::MT_DEBUG, QString("wDBTable unknown field name `%1' or (and) id `%1'\n").arg(str).arg(il[i]));
			}
		}
...
wDBTable::init(aDatabase *adb, aEngine *e )
{
	...
	aLog::print(aLog::MT_DEBUG, tr("wDBTable init in engine "));
//	printf("begin init wdbtable\n");
	...
	if ( !container )
	{	
		aLog::print(aLog::MT_ERROR, tr("wDBTable not in Ananas object container "));
	...
	else 
	{
		...
		{
			//debug_message("Table not found\n");
			aLog::print(aLog::MT_ERROR, tr("wDBTable init meta object not found "));
		}
		ctype = container->className();
		aLog::print(aLog::MT_INFO, tr("wDBTable container type is %1 ").arg(ctype));
	...
	if ( containerType() == "wDocument" )
	{
		...
		aLog::print(aLog::MT_INFO, tr("wDBTable filter is %1 ").arg(flt));
		setFilter(flt);
...
//				str = journalFieldName(lst[i].toLong());
	...
	refresh(RefreshAll);
	setWFieldEditor();
	aLog::print(aLog::MT_DEBUG, tr("wDBTable init in engine ok"));
}
...
//					printf("otupe = %s\n",str.ascii());
				...
				{
					aLog::print(aLog::MT_ERROR,tr("aEditorFactory field metaobject not found"));
				}
			}
	...
	{
		aLog::print(aLog::MT_ERROR,tr("aEditorFactory metadata in null"));
	}
...
{
	aLog::print(aLog::MT_ERROR, tr("wDBTable: function Value() call"));
	return QVariant("");
}
...
wDBTable::lineChange(int, int)
{
	...
	//}
	aLog::print(aLog::MT_INFO, tr("wDBTable: select document %1").arg(id));
...
wDBTable::contentsContextMenuEvent ( QContextMenuEvent * e )
{
	...
	if ( containerType() == "wJournal" )
	{
		...
		if ( !numRows() ) {
			popup->setItemEnabled(id[ IdView ], false );
			popup->setItemEnabled(id[ IdUpdate ], false );
			popup->setItemEnabled(id[ IdDelete ], false );
		}		
...
QDragObject * wDBTable::dragObject()
{
    if ( containerType() != "wCatalogue" ) return NULL;
    QSqlRecord * rec = currentRecord();
    if ( !rec ) return NULL;
    Q_ULLONG id = 0;
    if(rec->contains("id")) id = rec->value("id").toLongLong();
    ...
    if(!id) return NULL;
    aObject * aobj = new aObject(obj, db, this);
    if(!aobj->select(id)) return NULL;
    aObjectDrag * dobj = new aObjectDrag(aobj, this);
    dobj->setPixmap(systemIcon(), QPoint(1, 1));
    return dobj;
}

void wDBTable::dropped(QDropEvent *e)
{
    e->ignore();
}
...
aObjectDrag::aObjectDrag(aObject * obj, QWidget * dragSource) :
	QDragObject(dragSource, "aObjectDrag")
{
    fObj = obj;
}

aObjectDrag::~aObjectDrag()
{    
}

void aObjectDrag::setObject(aObject* obj)
{
    fObj = obj;
}

const char * aObjectDrag::format(int i) const
{
    if(i==0) return "application/aobject";
    return NULL;
}

QByteArray aObjectDrag::encodedData(const char*) const
{
    QByteArray ba(0);
    return ba;
}
