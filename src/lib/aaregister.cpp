/****************************************************************************
** $Id: aaregister.cpp,v 1.43 2008/01/21 06:40:54 app Exp $
**
...
aARegister::initObject()
{
	...
	if ( err ) 
	{
		setLastError( err, tr("Accumulation Register init"));
		return LastErrorCode();
	}
	err = tableInsert( db->tableDbName(*md,obj), obj );
	if ( err ) 
	{
		setLastError( err, tr("Accumulation Register create main table"));
		return LastErrorCode();
	}
	...
	for ( uint i = 0; i < n; i++  )
	{
		...
		if ( err ) 
		{
			setLastError( err, tr("Accumulation Register create saldo table"));
			return LastErrorCode();
		}
...
aARegister::Update()
{
	...
	aLog::print(aLog::MT_DEBUG, tr("Accumulation Register update"));
...
aARegister::deleteDocument( aDocument * doc )
{
	...
	if ( t->select(QString("idd=%1").arg(idd) ))
	{
		...
		aLog::print(aLog::MT_INFO, QString("Accumulation register delete document with idd=%1").arg(idd));
	}
	return true;
}
...
aARegister::getSaldo(const QString & strdate, const QString & dimfieldname, QVariant  dimvalue, const QString &resname)
{
	...
	if (!data.isValid()){
		...
		{
			setLastError( -1, tr("Accumulation register convert datetime from string %1").arg(strdate));
...
aARegister::getSaldo(const QDateTime &date, const QString & dimfieldname, QVariant dimvalue, const QString &resname)
{
	...
	if(!t_dim)
	{
		setLastError( -1, tr("Accumulation register not found table %1").arg(dimfieldname));
		return QVariant::Invalid;
	} 
	t_dim->clearFilter();
	if(!t_dim->setFilter(dimfieldname,dimvalue))
	{
		setLastError( -1, tr("Accumulation register set filter"));
		return QVariant::Invalid;
	}
	...
	{
		aLog::print(aLog::MT_DEBUG,"Accumulation register record empty");
	}
...
aARegister::getSaldoByManyDimensions(const QString &from, const QString &to, const QString & dimfieldname, QVariant dimvalue, const QString &resname)
{
	...
	if(!t)
	{
		setLastError( -1, tr("Accumulation register not found main table"));
		return QVariant::Invalid;
	} 
	...
	aLog::print(aLog::MT_INFO,QString("Accumulation register query %1").arg(query));
	QSqlQuery q = db->db()->exec(query);
	q.last();
	if(q.isValid())
	{
