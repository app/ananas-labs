...
aCalcTemplate::insertTagsValues(QDomNode node, const QString &tagName)
{
	...
	if(ok)
	{
		aLog::print(aLog::MT_DEBUG, QString("aCalcTemplate  append  double value %1 to tag").arg(val));
		...
		{
			aLog::print(aLog::MT_ERROR, "aCalcTemplate cell not found\n");
		}
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, "aCalcTemplate append string value to tag\n");
	}
