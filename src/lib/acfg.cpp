...
aCfg::read(const QString &fname){
    aLog::print(aLog::MT_DEBUG, QObject::tr("aCfg::read(const QString &fname) is running with fname = '%1'").arg(fname) );
    QFile file( fname );
    QByteArray buf;
    QString err;
    ...
    {
	aLog::print(aLog::MT_ERROR, QObject::tr("aCfg::read(const QString &fname) is running - Can't read file '%1'").arg(fname) );
    	return RC_ERROR;
    }
    buf = file.readAll();
    file.close();
	...
	aLog::print(aLog::MT_ERROR, QObject::tr(
		     "Error read configuration line:%1 col:%2 %3"
		     ).arg( errLine ).arg( errColumn ).arg( err ));
...
//	QString s = xml.toString(4).local8Bit(); // fake call for trap troble
	if ( !xml.isDocument() ) {
		aLog::print(aLog::MT_ERROR, tr("Bad metadata xml"));
		return;
	}
//CHECK_POINT
...
{
	aLog::print(aLog::MT_DEBUG,tr("aCfg::readrc(const QString &fname) is running with fname ='%1'.").arg(fname));
	int r = RC_ERROR;
	QString configfile;
	...
	{
		aLog::print(aLog::MT_ERROR,tr("aCfg::readrc(const QString &fname) - Empty 'configfile' configuration parameter."));
		r = RC_ERROR;
	}
	return r;
}
...
void
aCfg::createNew(){
	aLog::print(aLog::MT_DEBUG,tr(" aCfg::createNew() is running - Create new empty metadata schema."));
 ...
 *	- Catalogue - для объектов типа Каталог (примером каталога является Справочник товаров)
...
{
	aLog::print(aLog::MT_DEBUG, tr("aCfg::find(const QString &name) is running with name='%1'").arg(name));
	...
	gobj = md;
	aLog::print(aLog::MT_DEBUG, tr("aCfg::find(const QString &name) is running: length of metadata to search='%1'").arg(gobj.text().length()));
	while ( !oType.isEmpty() && !oName.isEmpty() ) {
		aLog::print(aLog::MT_DEBUG, tr("aCfg::find(const QString &name) choose type for %1").arg(oType));
		...
		if (oType == "Catalogue" || oType == tr("Catalogue") ) {
			aLog::print(aLog::MT_DEBUG, tr("aCfg::find(const QString &name) Catalogue type"));
			omType = md_catalogue;
	...
	aLog::print(aLog::MT_DEBUG, tr("aCfg::find(const QString &name) exit"));
...
{
	aLog::print(aLog::MT_DEBUG, tr("aCfg::findChild(aCfgItem context, const QString &classname, int n) is running with classname='%1'").arg(classname));
	if ( context.isNull() )
	{
		aLog::print(aLog::MT_ERROR, tr("Invalid parameter value in aCfg::findChild(aCfgItem context, const QString &classname, int n) 'context' should not be NULL"));
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aCfg::findChild(aCfgItem context, const QString &classname, int n) is running with context length='%1' ").arg(context.text().length()) );
	}
	int in = 0;
	aCfgItem i = firstChild( context );
	while ( !i.isNull() ) {
...
aCfg::swap(aCfgItem &item1, aCfgItem &item2)
{
	...
	{
		if(item1.isNull()) aLog::print(aLog::MT_DEBUG, tr("aCfg swap: first element is null"));
		else
		if(item2.isNull()) aLog::print(aLog::MT_DEBUG, tr("aCfg swap: second element is null"));
		else
		aLog::print(aLog::MT_ERROR, tr("aCfg swap element himself"));
		return false;
	}
	p1 = parent( item1 );
	p2 = parent( item2 );
	...
	{
		if(p1.isNull()) aLog::print(aLog::MT_DEBUG, tr("aCfg swap: first parent null"));
		else
		if(p2.isNull()) aLog::print(aLog::MT_DEBUG, tr("aCfg swap: second parent null"));
		else
		aLog::print(aLog::MT_ERROR, tr("aCfg swap elements with differernt parents"));
		return false;
	}
...
aCfg::saveOneObject(aCfgItem context, const QString &fileName)
{
	...
	if(!context.isNull())
	{
		...
		if(res==RC_OK)
		{
			aLog::print(aLog::MT_INFO, tr("aCfg saveOneObject: context save to file `%1'").arg(fileName));
		}
	}
	else
	{
		aLog::print(aLog::MT_ERROR, tr("aCfg saveOneObject: context is null"));
	}
	return res;
}
...
//		int errLine = 0, errColumn = 0;
		if ( file.open( IO_ReadOnly ) )
		{
			...
			{
				aLog::print(aLog::MT_ERROR, tr("aCfg loadOneObject: invalid cfg file `%1'").arg(fileName));
			}

		}
		else
		{
			aLog::print(aLog::MT_ERROR, tr("aCfg loadOneObject: invalid file name or not permission to file `%1'").arg(fileName));
		}
    	}
	return res;
}
...
aCfg::setupNewId( aCfgItem obj )
{
	if(!obj.isNull())
	{
		...
		{
			long id;
			aLog::print(aLog::MT_DEBUG, tr("aLog setup new id for %1").arg(attr(obj, mda_name)));
			id = nextID();
			aLog::print(aLog::MT_DEBUG, tr("aLog next id is %1").arg( id ) );
 ...
 *  Смотри также описание параметров метода find()
 * 
