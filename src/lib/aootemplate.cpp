...
aOOTemplate::open( const QString &fname )
{
	...
	aLog::print(aLog::MT_DEBUG, tr("aOOTemplate temporary directory is %1").arg(copyName));
//	printf("copy name = %s\n",copyName.ascii());
	if(!dir.mkdir(copyName))
	{
		aLog::print(aLog::MT_ERROR, tr("aOOTemplate create temporary directory %1").arg(copyName));
		return false;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aOOTemplate create temporary directory %1").arg(copyName));
	}
#ifndef Q_OS_WIN32
	...
	if( !process.start() )
	{
		aLog::print(aLog::MT_ERROR, tr("aOOTemplate start unzip"));
		return false;
	}
	...
	{
		aLog::print(aLog::MT_ERROR, tr("aOOTemplate unzip dead"));
		return false;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aOOTemplate unzip normal"));
	}
	...
	aLog::print(aLog::MT_INFO, tr("aOOTemplate open"));
return true;
}
/**
 * \en
...
aOOTemplate::getValue( const QString &name )
{
	...
	{
		aLog::print(aLog::MT_INFO, tr("aOOTemplate value for field %1 not setted").arg(name));
		return "";
	}
}
...
aOOTemplate::save( const QString & fname )
{
	...
	{
		aLog::print(aLog::MT_ERROR, tr("aOOTemplate save %1 open for write").arg(fContent.name()));
		return false;
	}
	...
	lineFeedRemove(fContent.name());	
	
	QFile fStyle( QDir::convertSeparators(copyName+"/styles.xml") );
	if( !fStyle.open( IO_WriteOnly ) )
	{
		aLog::print(aLog::MT_ERROR, tr("aOOTemplate save %1 open for write").arg(fStyle.name()));
		return false;
	}
	...
	lineFeedRemove(fStyle.name());	
	
	QString curDir;// = QDir::convertSeparators(QDir::irPath());
	curDir = QDir::convertSeparators(templateDir);
	aLog::print(aLog::MT_DEBUG, tr("aOOTemplate save working dir =%1").arg(curDir));
	...
	if( !process.start() )
	{
		aLog::print(aLog::MT_ERROR, tr("aOOTemplate start zip"));
		return false;
	}
	...
	if( !process.normalExit() ) 
	{
		aLog::print(aLog::MT_ERROR, tr("aOOTemplate zip dead"));
		return false;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aOOTemplate zip normal to %1").arg(fname));
	}
	if( process.exitStatus() )
	{
		return false;
 ...
 */
bool 
aOOTemplate::lineFeedRemove(const QString &filename)
{
	QFile 	file( filename );
	QString line;
	QString	str;
	...
	{
		aLog::print(aLog::MT_ERROR, tr("aOOTemplate lineFeedRemove cant open %1 in read mode").arg(filename));
		return false;
	}
	
	if( file.open( IO_WriteOnly ) ) 
	{
		file.writeBlock(str.utf8(), str.utf8().length() );
		file.close();
	}	
	else 
	{
