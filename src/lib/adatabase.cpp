/****************************************************************************
** $Id: adatabase.cpp,v 1.114 2008/10/26 18:57:16 leader Exp $
**
...
/**
 * /en
 *      \brief Creates database on SQL server. This is overloaded method, provided for convenience.
 *
 *      \param rcname (in) - rc file name
 *      \param dbname (in) - not used
 *      \return true, if no error
 * /_en /ru
 *      \brief Вспомогательный метод для установления соединения с БД на SQL сервере
 *      или ее создания.
 *
 *      Перегружет метод init( aCfgRc *rc, const QString &dbname ) и использует его.
 *      \param rcname (in) - имя файла ресурсов, содержащего параметры соединения с базой данных.
 *      \param dbname (in) - имя базы данных. (не используется, берется из файла ресурсов)
 *      \return true, если файл ресурсов успешно прочитан, false - в противном случае.
 * /_ru
 */
bool
...
aDatabase::loginUsersCount()
{
        ...
        else aLog::print ( aLog::MT_ERROR,tr ( "aDatabase get users count" ) );
        return 0;
}
