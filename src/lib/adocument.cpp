...
aDocument::New()
{
	if ( !sysJournal )
	{
		setLastError( err_nosysjournal, tr("aDocument have no sysjournal"));
		return LastErrorCode();
	}
	ERR_Code err = aObject::New();
	if ( err )
	{
		setLastError( err, tr("aDocument aObject::New() ended with error code = %1").arg(err));
		return LastErrorCode();
	}
	if ( obj.isNull() )
	{
		setLastError( err_noobject, tr("aDocument metaobject is null=%1"));
		return LastErrorCode();
	}
	Q_ULLONG Uid = getUid();
	SetPrefix(md->attr(obj,mda_name));
	aLog::print(aLog::MT_DEBUG, tr("aDocument new type = %1").arg(md->id(obj)));
	err = sysJournal->New( Uid, Prefix(), md->id(obj) );
	if ( err )
	{
		setLastError( err, tr("aDocument New() error while added record in sysJournal =%1").arg(err));
		table()->exec(QString("DELETE FROM %1 WHERE id=%2").arg(table()->tableName).arg(Uid));
		db->markDeleted( Uid );
		return err;
	}

	aLog::print(aLog::MT_DEBUG, tr("aDocument new document"));
	return err_noerror;
}
...
aDocument::TableNewLine( const QString & tablename )
{
	...
	if ( t->insert() )
	{
		...
		{
			setLastError( err_execerror, tr("aDocument select new table line number"));
			return LastErrorCode();
		}
		t->setSysValue("ln",ln);
		t->selected = true;
		aLog::print(aLog::MT_INFO, tr("aDocument new table line number=%1").arg(ln.toString()));
		return TableUpdate( tablename );
	}
	setLastError( err_inserterror, tr("aDocument insert new table line"));
	return LastErrorCode();
}
...
aDocument::Delete()
{
	...
	db->markDeleted( uid );
	aLog::print(aLog::MT_DEBUG, tr("aDocument delete from unicues"));
	aCfgItem tobj;
...
aDocument::TableDelete( const QString & tablename )
{
	if ( IsConducted() )
	{
		setLastError( err_docconducted, tr("aDocument delete table of conducted document"));
		return LastErrorCode();
	}
	aSQLTable * t = table(tablename);
	if ( !t )
	{
		setLastError( err_notable, tr("aDocument deleted table not exist"));
		return LastErrorCode();
	}
	if ( !t->selected )
	{
		//а эта проверка нужна???
		setLastError( err_notselected, tr("aDocument deleted table not selected"));
		return LastErrorCode();
	}
	...
	delete reg;
	aLog::print(aLog::MT_DEBUG, tr("aDocument delete table from info register"));
	...
	t->del();
	t->selected = false;
	aLog::print(aLog::MT_INFO, tr("aDocument delete table"));
	return err_noerror;
}
...
aDocument::tableDeleteLines( const QString & tablename )
{
	...
	if ( !dUid )
	{
		setLastError( err_notselected, tr("aDocument not selected")); 
		return LastErrorCode();
	}
	QString tdbname = table( tablename )->tableName;
	if ( tdbname == "" )
	{
		setLastError( err_notable, tr("aDocument table name is empty"));
		return LastErrorCode();
	}
...
aDocument::Update()
{
	...
	if ( e )
	{
		setLastError( e, tr("aDocument update error=%1").arg(e));
		return LastErrorCode();
	}
	e = sysJournal->Update();
...
aDocument::TableUpdate( const QString & tablename )
{
	if ( IsConducted() )
	{
		aLog::print(aLog::MT_DEBUG, tr("aDocument conducted table update"));
...
aDocument::setConduct( bool cond )
{
	...
	{
		aLog::print(aLog::MT_DEBUG, tr("aDocument conduct"));
		return err_noerror;
	}
	else
	{
		setLastError( err_conducterror, tr("aDocument conduct"));
		return LastErrorCode();
	}
}
...
ERR_Code
aDocument::Conduct(){
	aLog::print(aLog::MT_DEBUG, tr("Deprecated method call. aDocument::Conduct(...)"));
	return this->SignIn();
}
...
ERR_Code
aDocument::UnConduct(){
	aLog::print(aLog::MT_DEBUG, tr("Deprecated method call. aDocument::UnConduct()"));
	return this->SignOut();
}
...
aDocument::SignOut()
{
	...
	for ( uint i = 0; i < n; i++  )
	{
		...
		aLog::print(aLog::MT_DEBUG, tr("aDocument delete from accumulation register"));
		delete areg;
		areg = 0;
	}
	return err_noerror;
...
aDocument::IsConducted(){
	aLog::print(aLog::MT_DEBUG, tr("Deprecated method call. aDocument::IsConducted()"));
...
aDocument::GetDocumentValue(QVariant docId, const QString &fname)
{
	...
	{
		setLastError( -1, tr("aDocument::GetDocumentValue(...): Can't find aDocument"));
		return "Unknown";
	}
	//}
