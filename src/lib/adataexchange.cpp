/****************************************************************************
** $Id: adataexchange.cpp,v 1.16 2008/04/12 10:44:53 app Exp $
**
...
aBackup::importData(const QString& rcfile, const QString &archfile, bool dropBase, bool replaceTemplates)
{
	...
	{	
		...
		aLog::print(aLog::MT_ERROR, "aBackup create temporary directory %1");
		return true;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup create temporary directory %1").arg(tmpDirName));
	}
	if(unzipArchive(archfile, tmpDirName)==true)
	{
		...
		//printf("unzip error!\n");
		aLog::print(aLog::MT_ERROR, tr("aBackup import data: unzip"));
		return true;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup unzip"));
	}
	...
	aLog::print(aLog::MT_DEBUG, tr("aBackup filename = %1").arg(filename));
...
//	destDir.setPath(destDirName);
	...
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup template dir `%1' not exists, try create").arg(destDirName));
		if(!destDir.mkdir(destDirName,true))
		{
			aLog::print(aLog::MT_ERROR, tr("aBackup create template dir `%1' fail").arg(destDirName));
		}
		else
		{
			aLog::print(aLog::MT_DEBUG, tr("aBackup create template dir `%1'").arg(destDirName));
		}	
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup template dir `%1' exists").arg(destDirName));
	}
...
//	printf("copy %s to %s\n", QDir::convertSeparators(tmpDirName+"/busines-schema.cfg").ascii(), QDir::convertSeparators(filename+".cfg").ascii());
	if(!aService::copyFile( QDir::convertSeparators(tmpDirName+"/busines-schema.cfg"), QDir::convertSeparators(filename+".cfg"), true))
	{
		...
		aLog::print(aLog::MT_ERROR, tr("aBackup copy unzipped business schema file"));
		cleanupTmpFiles(tmpDirName, &templatesName);
		return true;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup copy business schema file"));
	}
	emit(progress(++prg,totalSteps));
//	printf("filename =%s\n",filename.ascii());
	...
	aLog::print(aLog::MT_INFO, tr("aBackup import data ok"));
	return false;
}
...
aBackup::exportData(const QString& rcfile, const QString &archfile, bool withTemplates )
{
	...
	if(!dir.mkdir(tmpDirName))
	{
		...
		aLog::print(aLog::MT_ERROR, tr("aBackup create temporary directory"));
		return true;
	}
	else
	{
	...
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup create temporary directory %1").arg(tmpDirName+"/META-INF"));
	}
	aCfg cfg;
	...
	if(cfg.readrc( rcfile ))
	{
		...
		aLog::print(aLog::MT_ERROR, tr("aBackup invalid *.rc file"));
		cleanupTmpFiles(tmpDirName, &templatesName);
		return true;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup valid *.rc file"));
	}
	qApp->processEvents();
	...
	if(withTemplates)
	{
		...
		aLog::print(aLog::MT_DEBUG, tr("aBackup workdir=%1").arg(srcDirName));
		dir.setPath(srcDirName);
		templatesName = dir.entryList("templ_*.odt;templ_*.ods");
		for(uint i=0; i<templatesName.count(); i++)
		{
			//ayTests::print2log("f:\\ERROR.log", "aBackup", tmpDirName + "/templates/"+templatesName[i]);
			aLog::print(aLog::MT_DEBUG, tr("aBackup template %1 %2").arg(i).arg(tmpDirName + "/templates/"+templatesName[i]));
//			file.remove();
		}
	}
	...
	if(cfg.write( tmpDirName+"/busines-schema.cfg" )) 
	{
		...
		aLog::print(aLog::MT_ERROR, tr("aBackup write %1 file").arg(tmpDirName+"/busines-schema.cfg"));
		cleanupTmpFiles(tmpDirName, &templatesName);
		return true;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup write %1 file").arg(tmpDirName+"/busines-schema.cfg"));
	...
	{
		cleanupTmpFiles(tmpDirName, &templatesName);
		aLog::print(aLog::MT_ERROR, tr("aBackup dump base error"));
		return true;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup bump base"));
	}
	...
	if(writeXml(QDir::convertSeparators(tmpDirName+"/META-INF/manifest.xml"), createManifest(templatesName))==true)
	{
		...
		aLog::print(aLog::MT_ERROR, tr("aBackup write manifest.xml"));
		cleanupTmpFiles(tmpDirName, &templatesName);
		return true;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup write manifest.xml"));
	}
...
//		setLastError(tr("Can't zip archive"));
		cleanupTmpFiles(tmpDirName, &templatesName);
		aLog::print(aLog::MT_ERROR, tr("aBackup zip archive"));
		return true;
	}
	else
	{
		aLog::print(aLog::MT_DEBUG, tr("aBackup zip archive"));
	}
	// remove files and directories
	cleanupTmpFiles(tmpDirName, &templatesName);
	emit (progress(++prg,totalSteps));
	setLastError(tr("Data export done without errors"));
	aLog::print(aLog::MT_DEBUG, tr("aBackup export data ok"));
...
//	printf("unzip to %s file `%s'\n",copyName.ascii(),fname.ascii());
	if( !process.start() )
	{
		...
		aLog::print(aLog::MT_ERROR, tr("aBackup start unzip"));
	...
	//	qWarning("FormTemplate::unzip(): error extracting document content");
		...
		aLog::print(aLog::MT_ERROR, tr("aBackup unzip dead"));
		return true;
	}
	else
	{
		//printf("process end normal\n");
		aLog::print(aLog::MT_DEBUG, tr("aBackup unzip normal"));
	}
...
aBackup::zipArchive(const QString& archName, const QString& dir)
{
	...
	if( !processUpdate.start() )
	{
		...
		aLog::print(aLog::MT_ERROR, tr("aBackup zip start error"));
		return true;
	}
	...
	if( !processUpdate.normalExit() )
	{
		...
		aLog::print(aLog::MT_ERROR, tr("aBackup zip dead"));
		return true;
	}
...
aBackup::dumpBase(const QString& rcfile, const QString& tmpDirName, int& prg, int totalSteps)
{
	...
	if(db.init(rcfile))
	{
		...
		db.exchangeDataUniques ( xml, false );
				
		aLog::print(aLog::MT_DEBUG, tr("aBackup dump tables ok"));
	}
	else
	{
		setLastError(tr("Can't connect to database"));
		aLog::print(aLog::MT_ERROR, tr("aBackup init rc file  %1").arg(rcfile));
		db.done();
		return true;
	}
	qApp->processEvents();
	...
	if(writeXml(tmpDirName+"/content.xml",xml)==true)
	{
		...
		aLog::print(aLog::MT_ERROR, tr("aBackup write content.xml"));
		return true;
	}
	else
	{
		aLog::print(aLog::MT_INFO, tr("aBackup dump base to xml compleet"));
	}
	return false;
}
...
aBackup::writeXml(const QString & name2Save, QDomDocument xml)
{
	...
	{
		aLog::print(aLog::MT_ERROR, tr("aBackup save xml %1").arg(name2Save));
		return true;
	}
	return false;	
}
...
aBackup::createManifest(const QStringList& templates)
{
	...
	aLog::print(aLog::MT_DEBUG, tr("aBackup create manifest.xml"));
	return manifest;	
}
...
aBackup::cleanupTmpFiles(const QString& tmpDirName, QStringList *files)
{
	...
	aLog::print(aLog::MT_DEBUG, tr("aBackup delete file %1").arg(file.name()));
	file.remove();
	...
	aLog::print(aLog::MT_DEBUG, tr("aBackup delete directory %1").arg(tmpDirName + "/META-INF"));
	dir.rmdir(QDir::convertSeparators(tmpDirName + "/META-INF"));
	aLog::print(aLog::MT_DEBUG, tr("aBackup delete directory %1").arg(tmpDirName + "/templates"));
	dir.rmdir(QDir::convertSeparators(tmpDirName + "/templates"));
	aLog::print(aLog::MT_DEBUG, tr("aBackup delete directory %1").arg(tmpDirName));
	dir.rmdir(QDir::convertSeparators(tmpDirName));
	aLog::print(aLog::MT_INFO, tr("aBackup cleanup temporary files"));
