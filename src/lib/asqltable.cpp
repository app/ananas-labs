/****************************************************************************
** $Id: asqltable.cpp,v 1.79 2008/01/21 06:46:26 app Exp $
**
...
aDataTable::setObject( aCfgItem context )
{
	if ( context.isNull() )
	{
		aLog::print(aLog::MT_ERROR,QObject::tr("aDataTable try set mdobject to null"));
		return;
	}
...
aDataTable::value ( const QString & name )
{
	...
	if ( !fnames[name] )
	{
		aLog::print(aLog::MT_ERROR, QObject::tr("aDataTable get value of unknown field `%1'").arg(name));
...
aDataTable::setSysValue ( const QString & name, QVariant value )
{
	...
	{
		aLog::print(aLog::MT_INFO, QObject::tr("aDataTable get document prefix to `%1'").arg(value.toString()));
...
{
	aLog::print(aLog::MT_DEBUG, QObject::tr("aDataTable clear filter"));
	userFilter.clear();
}
...
aDataTable::setFilter ( const QString & newFilter ) {
	QSqlCursor::setFilter( newFilter );
}
...
aDataTable::setFilter( const QString& name, const QVariant& value )
{
	...
	aLog::print(aLog::MT_DEBUG, QObject::tr("aDataTable set filter %1='%2'").arg(name).arg(value.toString()));
	if ( !fnames[name] )
	{
		aLog::print(aLog::MT_ERROR, QObject::tr("aDataTable set filter %1='%2', %3 not exist").arg(name).arg(value.toString()).arg(name));
		return false;
	}
	QVariant *v = new QVariant(value);
	userFilter.replace(*fnames[name], v );
	QSqlCursor::setFilter(getFilter());
	aLog::print(aLog::MT_DEBUG, QObject::tr("aDataTable QSqlCursor filter set to '%1'").arg( getFilter() ) );
...
aDataTable::New()
{
	...
	{
		Uid = db->uid( mdobjId );
		aLog::print(aLog::MT_DEBUG, QString("aDataTable new record with id=%1 for meta object with id=%2").arg(Uid).arg(mdobjId));
	...
	if ( insert() )
	{
		if ( select(QString("id=%1").arg(Uid), false) )
			...
			{
				aLog::print(aLog::MT_ERROR, QString("aDataTable record with id=%1 not found").arg(Uid));
			}
	}
	return res;
}
...
/**
 * \ru
 * 	\brief	Создает новую запись, являющуюся клоном текущей записи.
 * \_ru
 */
bool
aDataTable::Copy()
...
aDataTable::Update()
{
	...
	{
		aLog::print(aLog::MT_ERROR, QString("%1 %2").arg(err.text()).arg(err.driverText()) );	
	}
	return true;
}
...
// 	return  ( fnames[userFieldName] ) ? (*fnames[ userFieldName ]) : "";
}
