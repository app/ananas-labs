/****************************************************************************
** $Id: airegister.cpp,v 1.38 2008/01/21 06:42:03 app Exp $
**
...
aIRegister::deleteDocument( aDocument * doc )
{
	...
	if ( iRegs.isNull() )
	{
		setLastError( -1, tr("aIRegister metaobject is null"));
		return false;
	}
	Q_ULLONG idd = doc->getUid();
	if ( !idd )
	{
		setLastError( -1, tr("aIRegister deleted document have invalid idd"));
		return false;
	}
...
{
	//printf("from %s, to %s\n",from.ascii(),to.ascii() );
	aLog::print(aLog::MT_DEBUG, tr("aIRegister select period from %1 to %2").arg(from).arg(to));
